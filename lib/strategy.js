...
function Strategy(options, verify) {
  ...
  this._userNameField = options.userEmail || 'userName'
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (!userEmail || !password || !profileId || !userName) {
  ...
  try {
    if (self._passReqToCallback) {
      this._verify(req, userEmail, userName, password, profileId, verified)
    } else {
      this._verify(userEmail, userName, password, profileId, verified)
    }
  } catch (ex) {
